...
class SaveModel(pl.callbacks.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if epoch % 10 == 0:
            pl_module.save_to(os.path.join(self.log_dir, "{}.nemo".format(epoch)))
        

@hydra_runner(config_path = "conf", config_name = "SpeakerNet.yaml")
def main(cfg):

    # logging.info(f'Hydra config: {OmegaConf.to_yaml(cfg)}')
    trainer = pl.Trainer(**cfg.trainer)
    log_dir = os.path.join(exp_manager(trainer, cfg.get("exp_manager", None)), "checkpoints")
    
    trainer = pl.Trainer(**cfg.trainer, callbacks = [SaveModel(log_dir = log_dir)])
