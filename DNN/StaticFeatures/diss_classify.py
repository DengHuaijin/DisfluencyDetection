...
def Train(save_model_name: str, num_set: str, detection: str, dataset: str, tag: str, load = False):
    ...
    # train_max = max([len(i) for i in x_train_lstm]) 
    # test_max = max([len(i) for i in x_test_lstm])
    ...
    #x_train_svm = np.zeros(shape = x_train_svm.shape)
    ...
    print("FUNC Train max: {} min: {}  Test max: {} min: {}".format(max(x_train_lstm[:, 1]), min(x_train_lstm[:, 1]), 
                                                                   max(x_test_lstm[:, 1]), min(x_test_lstm[:, 1])))
    print("Class weights: ", class_weight)
    if load:
        model = DNN_Model(load = load, save_model_name = save_model_name, 
                          num_set = num_set, detection = detection)
    else:
        input_dims = x_train_lstm.shape[1]
        time_steps = None
...
def Test(load_model_name: str, num_set: str, detection: str, dataset: str, tag: str, csvfile = None, load = False):
