...
LR_SCHEDULED = [
        (10, 0.05), (20, 0.01), (30, 0.001), (40, 0.0005), (50, 0.00001), 
        (60, ), (70, ), (80, ), (90, ), (70, ), (80, ), (90, )]

def lr_schedule(epoch, lr):
    
    """
    if epoch < LR_SCHEDULED[0][0] or epoch > LR_SCHEDULED[-1][0]:
        return lr
    for i in range(len(LR_SCHEDULED)):
        if epoch == LR_SCHEDULED[i][0]:
            return LR_SCHEDULED[i][1]
    """
    initial_lr = lr
    """
    # exp decay
    ...
    drop = 0.00046
    epochs_drop = 20
    lrate = initial_lr * np.power(drop, np.floor((epoch) / epochs_drop))
...
def train_test_model(hparams, num_set, session_num):
    ...
    logdir = os.path.join("tensorboard_log", detection, "tuning_lr", str(session_num), num_set)
    ...
    history = model.fit(x = train_set,
                        ...
                        callbacks = [tensorboard_callback, LearningRateSchedule(lr_schedule)])
