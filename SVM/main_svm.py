        ...
        for j in param_C:
            for k in param_gamma:
                # for l in param_degree:
                    params = {"kernel": i, "C": j, "gamma": k, "degree": 2, "class_weight": None, 'cache_size': cache_size}
                    ...
                    if metrics == "f1":
                        ...
                        if detection == "negative":
                            if precision_mean > precision_best and neg_mean > 0.3:
                                precision_best = precision_mean
                                params_best = params
                        elif detection == "positive":
                            if precision_mean > precision_best and pos_mean > 0.3:
                                precision_best = precision_mean
                                params_best = params
                    
                    elif metrics == "neg_acc":
                        if neg_mean > neg_best and acc_mean > 0.5:
                            neg_best = neg_mean
                            params_best = params
                    
                    elif metrics == "pos_acc":
                        if pos_mean > pos_best and acc_mean > 0.5:
                            pos_best = pos_mean
                            params_best = params
                    # print("kernel: {}  C: {}  gamma: {}  Acc:{:.3f}  Neg Acc: {:.3f}  Pos Acc: {:.3f}".format(params["kernel"], params["C"], params["gamma"], acc_mean, neg_mean, pos_mean))
